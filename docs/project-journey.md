...
### 2026-04-15 — From metric soup to an honest story: defending k=4 in Notebook 3

We streamlined the k-scan workflow in the RFM clustering notebook so one cell owns the full **k=2…7** evaluation: inertia with drop rates, separation metrics, and downstream aliases (`k_eval`, `labels_by_k`) without duplicate calculations. That small refactor matters operationally because notebooks that recompute the same table twice invite drift between outputs and stale variable state when cells are run out of order.

On the analytics side, we treated **k=4** as the operational choice aligned with four planted behavioral segments and documented the defense using standard rules of thumb: **elbow-style diminishing returns** after k=4, silhouette compared across neighboring k values, and Davies–Bouldin and Calinski–Harabasz read as secondary signals rather than a single score to maximize. We also integrated **silhouette “blades”** and **per-feature boxplots**—not to pretend separation is perfect, but to show where clusters overlap (e.g. ticket size and product counts) and where differentiation actually lives (frequency and activity ratios). The key insight is that **messy overlap on some margins is normal in wallet behavior**; what makes segmentation credible is pairing metrics with transparent profile plots and a clear business reason for k, not a pristine Voronoi diagram.

### 2026-04-16 — Tightening feature scope and expanding profile diagnostics

Today we made two practical improvements to keep Notebook 3 more reliable for decision-making. First, we rolled back a temporary product-level share extension in the feature-engineering module, returning to the agreed monetary-type share design so the clustering input stays aligned with the documented baseline and avoids accidental feature creep. This matters because unsupervised results are very sensitive to schema drift, and even "small" extra columns can silently change distance geometry and downstream interpretations.

Second, we upgraded the cluster profiling view from a manually curated list of seven variables to an automatic boxplot panel over all numeric model features. That gives a fuller, less biased picture of where clusters are truly separated versus where they overlap, without requiring manual maintenance every time the feature set evolves. The key lesson is that robust segmentation work needs both controls and coverage: strict feature governance on what enters the model, and broad diagnostics on how those features behave after clustering.

### 2026-04-24 — Designing an AI-ready customer mart contract

Today we reframed Notebook 3's `customer_analysis` table from a clustering output into a production contract for an automated recommendation agent. Instead of sending every available metric to the LLM, we separated concerns: the mart keeps a wide analytical schema for reproducibility and diagnostics, while the agent receives a curated subset of high-signal business features. We also resolved an important governance question around lifecycle handling, keeping `churned_pre_window` because it represents a real behavioral state that requires different actions from customers who never activated.

The main learning was that recommendation quality depends as much on input design as model choice. We defined explicit agent-facing fields for segmentation, value, product ownership, and lifecycle context, and introduced a clear flag (`llm_agent`) to mark which columns are allowed into prompts. This turns the table into both a data product and an operational policy: analysts can keep rich diagnostics in the mart, while the agent stays focused on a stable, explainable input set.

### 2026-04-24 — Making Notebook 3 navigation and handoff production-ready

Today we upgraded Notebook 3 from a strong analysis artifact into a clearer operational handoff document. We rebuilt the first-cell summary so the notebook flow matches the real execution sequence end-to-end, fixed fragile internal links by standardizing anchor IDs, and added explicit navigation targets for all major sections. This matters because long notebooks are only useful for teams if people can reliably move from overview to evidence without manual searching or broken jumps.

We also strengthened the project narrative at the top of the notebook: beyond RFM clustering and profiling, the introduction now explains the next delivery step—using an LLM agent to convert insights from Notebooks 1, 2, and 3 into next-best-action recommendations. The key lesson is that analytical quality is not only about model metrics; documentation structure, traceable flow, and a clear downstream action path are what make the work reusable in real business operations.
